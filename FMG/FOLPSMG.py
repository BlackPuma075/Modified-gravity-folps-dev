...
def fOverf0EH(zev, k, OmM0, h, fnu, fr0):
    ...
    '''if (fnu > 0):
        ...
        rf = DEdS/(1 + yFS)
        #fFit = 1 - pcb/(1 + (rf)**c)
        fFit = 1+0.19*nu**np.exp((lmbda/(nu*np.log(nu)))*(k+np.log10(1e-5)/2+4))-pcb/(1+(rf)**c)  #f(k)/f0
    ...
    if (fnu > 0):   
        ...
        etaini = -5;  #initial eta, early enough to evolve as EdS (D + \propto a)
        zfin = -0.99;
        fr0 = 1e-5;
        invH0 = 2997.92458;
        beta2 = 1/6;
        ...
        etaev = 0
        OmM0 = 0.3
        #etastep = (etaev-etaini)/100 

        from scipy.integrate import odeint
        from scipy.interpolate import interp1d

        #Differential eqs.
        def Deqs(Df, eta, k, OmM0, fR0): #Diff. equations
            Df, Dprime = Df
            return [Dprime,  -f1(eta)*Dprime + mu(eta, k, OmM0, fR0)*f2(eta)*Df]

        def interpolador(eta, etaarray, D):
            interp_func = interp1d(etaarray, D, kind='linear', fill_value="extrapolate")
            return interp_func(eta)
        
        def etaofz(z):
            return np.log(1 / (1 + z))
    
        etafin = etaofz(zfin)
        etaarray = np.linspace(etaini, etafin, 50)
        Df0 = np.exp(etaini) #Initial conditions
        Df_p0 = np.exp(etaini)
        Dp = []
        Dpp = []
        #kTout = kmin * np.exp(np.linspace(0, np.log(kmax/kmin), Nk))
        for i in range(len(k)):
            Dplus, Dplusp = odeint(Deqs, [Df0,Df_p0], etaarray, args = (k[i], OmM0, fr0)).T
            Df_sol = interpolador(etaofz(zev), etaarray, Dplus)
            Dprime = interpolador(etaofz(zev), etaarray, Dplusp)
            Dp = np.append(Dp, Df_sol)
            Dpp = np.append(Dpp, Dprime)
        
        fFit = Dpp/Dp
        f0 = fFit[0]
        
        Dplus_LCDM, Dplusp_LCDM = odeint(Deqs, [Df0, Df_p0], etaarray, args=(0, OmM0, 1e-15)).T
        
        interp_LCDM = interpolador(etaofz(zev), etaarray, Dplus_LCDM)
        
    return (k, fFit/f0, f0, (Dp/interp_LCDM)**2)    
...
def NonLinear(inputpkl, CosmoParams, kminout=0.001, kmaxout=0.5, nk = 120, EdSkernels = False):
    ...
    global z_pk, omega_b, omega_cdm, omega_ncdm, h, fr0
    ...
    inputfkT = fOverf0EH(z_pk, inputpkT[0], OmM0, h, fnu, fr0)
    f0 = inputfkT[2]
    inputpkT = np.array([inputpkT[0], inputpkT[1]*inputfkT[3]])
