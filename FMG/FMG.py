...
def select_strategic_points(k, num_points=30):
    
    k_filtered = k[(k > k[0]) & (k < k[-1])]
   
    if len(k_filtered) <= num_points:
        return k_filtered
    
    k_filtered = np.sort(k_filtered)
    
    indices = np.round(np.linspace(0, len(k_filtered) - 1, num_points)).astype(int)
    k_selected = k_filtered[indices]
    
    return k_selected


def fOverf0MG(k_array, zev, OmM0, fR0):
    ...
    def mass(eta, OmM0, fR0):
        nHS = 1
        invH0 = 2997.92458  # 1/H0 in Mpc/h units
        return (1 / invH0) * (1 / (2 * fR0))**(1/2) * \
               (OmM0 * np.exp(-3 * eta) + 4 * (1 - OmM0))**((2 + nHS) / 2) / \
               (OmM0 + 4 * (1 - OmM0))**((1 + nHS) / 2)
    
    def mu(eta, k, OmM0, fR0):
        beta2 = 1 / 6
        return 1 + (2 * beta2 * k**2) / (k**2 + np.exp(2 * eta) * mass(eta, OmM0, fR0)**2)
    
    def etaofz(z):
        return np.log(1 / (1 + z))
    
    def equation(D, eta, k, OmM0, fR0):
        Df, Df_prime = D
        mu_val = mu(eta, k, OmM0, fR0)
        dDf_prime = f2(eta) * mu_val * Df - f1(eta) * Df_prime
        return [Df_prime, dDf_prime]
    
    
    k_selected = select_strategic_points(k_array)
    #print(len(k_selected))
    
    etaini = -5
    zfin = -0.99
    etafin = etaofz(zfin)
    
    eta = np.linspace(etaini, etafin, 50)
    Df0 = np.exp(etaini)
    Df_p0 = np.exp(etaini)
    
    Dplus = np.zeros((len(eta), len(k_selected)))
    Dplusp = np.zeros((len(eta), len(k_selected)))
    
    for i, k in enumerate(k_selected):
        # Solve ODE for each k
        results = odeint(equation, [Df0, Df_p0], eta, args=(k, OmM0, fR0))
        Dplus[:, i] = results[:, 0]
        Dplusp[:, i] = results[:, 1]
    
    interp_zev_Dplus = interp(etaofz(zev), eta, Dplus)
    interp_zev_Dplusp = interp(etaofz(zev),  eta, Dplusp)
    
    interp_Dplus = interp(k_array, k_selected, interp_zev_Dplus)
    interp_Dplusp = interp(k_array, k_selected, interp_zev_Dplusp)
    
    Dplus_LCDM, Dplusp_LCDM = odeint(equation, [Df0, Df_p0], eta, args=(0, OmM0, 1e-15)).T
        
    fk =  interp_Dplusp/interp_Dplus
    f0 = fk[0]
    
    interp_zev_Dplus_LCDM = interp(etaofz(zev), eta, Dplus_LCDM)
    #print(fk)
    
    return k_array, fk/f0, f0, (interp_Dplus/interp_zev_Dplus_LCDM)**2
...
def NonLinear(inputpkl, CosmoParams, kminout=0.001, kmaxout=0.5, nk = 120, EdSkernels = False):
    ...
    Args:
        ...
        CosmoParams: Set of cosmological parameters [z_pk, omega_b, omega_cdm, omega_ncdm, h] in that order.
                   ...
                   h = H0/100. 
        kminout: Minimum k value for the output spectrum.
        kmaxout: Maximum k value for the output spectrum.
        nk: Number of k values.
        EdSkernels: If True, EdS-kernels will be employed. If False, fk-kernels will be used.
        fR0: Optional parameter for modified gravity models. If None, it is ignored.
    ...
    global z_pk, omega_b, omega_cdm, omega_ncdm, h, rdrag, fR0_global

    #CosmoParams
    (z_pk, omega_b, omega_cdm, omega_ncdm, h, fR0) = CosmoParams
    ...
    ############  rdrag in Mpc/h #################
    rdrag = (55.154 *h)* np.exp(-72.3*(0.0006 + omega_ncdm)**2) / omega_b**0.12807 / (omega_b + omega_cdm)**0.25351
	# formula from https://arxiv.org/abs/1411.1074    
    ...
    if fR0_global is not None:
        inputfkT = fOverf0MG(k_array=inputpkT[0], zev=z_pk, OmM0=OmM0, fR0=fR0_global)
        D2_ratio = inputfkT[3]
        f0 = inputfkT[2]
        inputpkT = np.array([inputpkT[0], inputpkT[1] * D2_ratio])
        #print('fR model')
    else:
...
def TableOut_interp(k, growth_D_zold = 1.0, growth_D_znew = 1.0):
    ...
    nobjects = 23 + n_A
    Tableout = np.zeros((nobjects + 3, len(k)))
    for ii in range(nobjects):
        Tableout[0][:] = interp(k, kTout, TableOut[1]) * (growth_D_znew/growth_D_zold) **2        #pkl
        Tableout[1][:] = interp(k, kTout, TableOut[2])                                            #fk/f0
        Tableout[2+ii][:] = interp(k, kTout, TableOut[3+ii]) * (growth_D_znew/growth_D_zold) **4
        Tableout[25+n_A][:] = sigma2w * (growth_D_znew/growth_D_zold) **2
    return Tableout




def TableOut_NW_interp(k, growth_D_zold = 1.0, growth_D_znew = 1.0):
    ...
    n_A = 6 if Afull else 0
    nobjects = 23 + n_A
    Tableout_NW = np.zeros((nobjects + 3, len(k)))
    for ii in range(nobjects):
        Tableout_NW[0][:] = interp(k, kTout, TableOut_NW[1]) * (growth_D_znew/growth_D_zold) **2       #pkl
        Tableout_NW[1][:] = interp(k, kTout, TableOut_NW[2])                                           #fk/f0
        Tableout_NW[2+ii][:] = interp(k, kTout, TableOut_NW[3+ii]) * (growth_D_znew/growth_D_zold) **4
        Tableout_NW[25+n_A][:] = sigma2w_NW * (growth_D_znew/growth_D_zold) **2
    return Tableout_NW




def PEFTs(kev, mu, f0, NuisanParams, Table):
    ...
    Args: 
        ...
        f0: cosmic growth factor at very large scales, or simply the growth factor in models without massive neutrinos.
            computed at the new redshift.
...
def Sigma2Total(kev, mu, f0, Table_NW):
...
def RSDmultipoles(kev, z_pk, f0, NuisanParams, growth_D_zold = 1.0, growth_D_znew = 1.0, Omfid = -1, AP = False,
                  IR = True, bias="folps"):
    ...
    def PIRs(kev, mu, Table, Table_NW):
        ...
        if AP == True:
            ...
            if IR == True:
                Sigma2T = Sigma2Total(k_true, mu_true, f0, Table_NW_true)
            else:
                Sigma2T = 0
            ...
            return ((b1 + fk * mu_true**2)**2 * (pkl_NW + np.exp(-k_true**2 * Sigma2T)*(pkl-pkl_NW)*(1 + k_true**2 * Sigma2T) )
                + np.exp(-k_true**2 * Sigma2T)*PEFTs(k_true, mu_true, f0, NuisanParams, Table_true) 
                + (1 - np.exp(-k_true**2 * Sigma2T))*PEFTs(k_true, mu_true, f0, NuisanParams, Table_NW_true)) 
            ...
            if IR == True:
                Sigma2T = Sigma2Total(kev, mu, f0, Table_NW)
            else:
                Sigma2T = 0
            
            return ((b1 + fk * mu**2)**2 * (pkl_NW + np.exp(-k**2 * Sigma2T)*(pkl-pkl_NW)*(1 + k**2 * Sigma2T) )
                + np.exp(-k**2 * Sigma2T)*PEFTs(k, mu, f0, NuisanParams, Table) 
                + (1 - np.exp(-k**2 * Sigma2T))*PEFTs(k, mu, f0, NuisanParams, Table_NW))
    ...
    Pkl0 = ModelPkl0(TableOut_interp(kev, growth_D_zold, growth_D_znew),
                     TableOut_NW_interp(kev, growth_D_zold, growth_D_znew));
    Pkl2 = ModelPkl2(TableOut_interp(kev, growth_D_zold, growth_D_znew),
                     TableOut_NW_interp(kev, growth_D_zold, growth_D_znew));
    Pkl4 = ModelPkl4(TableOut_interp(kev, growth_D_zold, growth_D_znew),
                     TableOut_NW_interp(kev, growth_D_zold, growth_D_znew));
...
def RSDmultipoles_marginalized_const(kev, z_pk, f0, NuisanParams, growth_D_zold = 1.0, growth_D_znew = 1.0,
                                     Omfid = -1, AP = False, Hexa = False, bias="folps"):
    ...
    def PIRs_const(kev, mu, Table, Table_NW):
        ...
        if AP == True:
            ...
            return ((b1 + fk * mu_true**2)**2 * (pkl_NW + np.exp(-k_true**2 * Sigma2T)*(pkl-pkl_NW)*(1 + k_true**2 * Sigma2T) )
                + np.exp(-k_true**2 * Sigma2T)*PEFTs(k_true, mu_true, f0, NuisanParams_const, Table_true) 
                + (1 - np.exp(-k_true**2 * Sigma2T))*PEFTs(k_true, mu_true, f0, NuisanParams_const, Table_NW_true))
            ...
            return ((b1 + fk * mu**2)**2 * (pkl_NW + np.exp(-k**2 * Sigma2T)*(pkl-pkl_NW)*(1 + k**2 * Sigma2T) )
                + np.exp(-k**2 * Sigma2T)*PEFTs(k, mu, f0, NuisanParams_const, Table) 
                + (1 - np.exp(-k**2 * Sigma2T))*PEFTs(k, mu, f0, NuisanParams_const, Table_NW))
    ...
    if Hexa == False:
        Pkl0_const = ModelPkl0_const(TableOut_interp(kev, growth_D_zold, growth_D_znew),
                                     TableOut_NW_interp(kev, growth_D_zold, growth_D_znew));
        Pkl2_const = ModelPkl2_const(TableOut_interp(kev, growth_D_zold, growth_D_znew),
                                     TableOut_NW_interp(kev, growth_D_zold, growth_D_znew));
        ...
        Pkl2_const = ModelPkl2_const(TableOut_interp(kev, growth_D_zold, growth_D_znew),
                                     TableOut_NW_interp(kev, growth_D_zold, growth_D_znew));
        Pkl4_const = ModelPkl4_const(TableOut_interp(kev, growth_D_zold, growth_D_znew),
                                     TableOut_NW_interp(kev, growth_D_zold, growth_D_znew));
...
def RSDmultipoles_marginalized_derivatives(kev, z_pk, f0, NuisanParams, growth_D_zold = 1.0, growth_D_znew = 1.0,
                                     Omfid = -1, AP = False, Hexa = False, bias="folps"):
    ...
    if Hexa == False:   
        Pkl0_derivatives = ModelPkl0_derivatives(TableOut_interp(kev, growth_D_zold, growth_D_znew),
                                                 TableOut_NW_interp(kev, growth_D_zold, growth_D_znew));
        Pkl2_derivatives = ModelPkl2_derivatives(TableOut_interp(kev, growth_D_zold, growth_D_znew),
                                                 TableOut_NW_interp(kev, growth_D_zold, growth_D_znew));
        ...
        Pkl2_derivatives = ModelPkl2_derivatives(TableOut_interp(kev, growth_D_zold, growth_D_znew),
                                                 TableOut_NW_interp(kev, growth_D_zold, growth_D_znew));
        Pkl4_derivatives = ModelPkl4_derivatives(TableOut_interp(kev, growth_D_zold, growth_D_znew),
                                                 TableOut_NW_interp(kev, growth_D_zold, growth_D_znew));
